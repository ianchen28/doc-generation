...
# 导入Nacos配置
try:
    from doc_agent.config.nacos_config import config_file as nacos_config
except ImportError:
    nacos_config = {}
...
class ModelConfig(BaseSettings):
    ...
    model_name: str = Field(alias="model_id")  # 使用别名避免命名冲突
    url: str
    api_key: str
    description: str
...
class ElasticsearchConfig(BaseSettings):
    ...
    # 新增连接池配置
    connections_per_node: int = 25  # 每个节点的连接数（推荐配置）
...
class AgentConfig(BaseSettings):
    """Agent整体配置"""
    default_llm: str = "qwen_2_5_235b_a22b"
...
class AppSettings(BaseSettings):
    ...
    def _load_yaml_config(self):
        """加载配置文件（优先从Nacos，降级到YAML）"""
        # 首先尝试从Nacos获取配置
        if nacos_config:
            print(f"从Nacos获取配置: {nacos_config}")
            self._yaml_config = nacos_config
            return
        
        # 如果Nacos配置为空，降级到YAML文件
    ...
    def server_config(self) -> dict[str, Any]:
        """获取服务器配置"""
        if self._yaml_config and 'server' in self._yaml_config:
            return self._yaml_config['server']
        else:
            # 默认配置
            return {
