...
async def test_researcher_user_docs():
    """测试 researcher.py 中用户文档搜索功能"""
    logger.info("开始测试 researcher.py 中用户文档搜索功能")
    ...
    try:
        ...
        # 执行 researcher 节点
        result = await async_researcher_node(
            state=test_state,
        ...
        logger.info(f"  - 总信源数量: {len(gathered_sources)}")
        logger.info(f"  - 用户参考文档信源数量: {len(user_data_sources)}")
        logger.info(f"  - 用户风格指南信源数量: {len(user_style_sources)}")
        logger.info(f"  - 用户需求文档信源数量: {len(user_requirement_sources)}")
        
        # 显示详细信息
        if user_data_sources:
            logger.info("用户参考文档信源详情:")
            for i, source in enumerate(user_data_sources[:3], 1):
                logger.info(f"  {i}. ID: {source.id}, 标题: {source.title[:50]}...")
                logger.info(f"     类型: {source.source_type}, 来源: {source.source}")
        ...
        # 验证结果
        if gathered_sources:
            logger.info("✅ 测试成功：找到了用户文档搜索结果")
        else:
            logger.warning("⚠️ 测试警告：没有找到任何搜索结果")
...
if __name__ == "__main__":
    # 运行测试
    asyncio.run(test_researcher_user_docs())
